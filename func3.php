...
<?php $sql = "SELECT * FROM users WHERE 1";
				...
				$test=0;
                while ($row = mysqli_fetch_assoc($result) and $ok==0) {
                  if ($row['username']==$username and $row['functie']=="ADMIN"){$ok=$ok+1;
                 ?>
  <div class="card-elevi">
    <div class="header-pagina" style="margin-bottom: 40px; color: #FF00FF; font-weight: bold;"><b>CATALOG</div></b>

    <form action="vezi_unitate.php" method="POST">
      <div class="form-group">
        <label class="col-sm-3 control-label"><b>UNITATE:</b></label>
        <div class="col-sm-6">
          <select name="unitate" id="unitate">
            ...
            $ok = 0;$test=0;
            while ($row = mysqli_fetch_assoc($result)) { ?>
			<option><?php if($row['id']==$scoala){ echo $row['nume_partener'];} ?> </option>
            <?php } ?>
          </select>
        </div>
      </div>
      ...
      <center><input class="buton-triforce" type="submit" value="AFISEAZA LISTA CU NOTE"></center>
				</form> <?php $test=$test+1; }}if($ok==0 and $test==0){?>
				<div class="card-elevi">
    <div class="header-pagina" style="margin-bottom: 40px;">CATALOG</div>
      ...
      <div class="form-group">
        ...
        <div class="col-sm-6">
          <select name="unitate" id="unitate">
            ...
            $result = mysqli_query($conectare, $sql);
            $k = 0;
            while ($row = mysqli_fetch_assoc($result) and $k==0) {
                   if($scoala==$row['id']){$k=$k+1;				?>
              <option><?php  echo $row['nume_partener']; ?> </option>
				
          </select>
        </div>
      </div>

      <center><input class="buton-triforce" type="submit" value="AFISEAZA LISTA CU NOTE"></center></form> <?php }}}?>
  </div>
