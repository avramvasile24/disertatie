...
<?php $unitate=$_POST['unitate']; ?>
  <div class="card-elevi">
    <div class="header-pagina" style="margin-bottom: 40px;">CATALOG - <?php echo $unitate; ?></div>
     <?php $sql = "SELECT * FROM users WHERE 1";
            $result = mysqli_query($conectare, $sql);
            $ok = 0;
			$prof=0;
            while ($row = mysqli_fetch_assoc($result)) { if($row['username']==$_SESSION['username'] and $row['functie']=='ELEV'){$clasa=$row['clasa'];}} if($clasa==null){$prof=1;}?>
	 <center><div class='buton-triforce'><a href='func3.php'><i class='fa fa-back'></i> REVENIRE LA CATALOG</a></div>
	<div class='buton-triforce'><a href='dashboard.php'><i class='fa fa-back'></i> REVENIRE LA PAGINA PRINCIPALA</a></div>
	</center>
	<?php $sql = "SELECT * FROM parteneri WHERE 1";
            $result = mysqli_query($conectare, $sql);
            $ok = 0;
            while ($row = mysqli_fetch_assoc($result)) { if($row['nume_partener']==$unitate){ $idUnitate=$row['id'];}}?>
    <form action="vezi_note.php" method="POST">
      <div class="form-group">
        <label class="col-sm-3 control-label"><b>CLASA:</b></label>
        <div class="col-sm-6">
          <select name="clasa" id="clasa">
			...
			$stop=0;
			echo $prof;
            while ($row = mysqli_fetch_assoc($result)){ if($prof!=0){ if($row['idUnitate']==$idUnitate) {?><option><?php echo $clasa = $row['cys']; ?> </option><?php } } else if($prof==0) { if($row['idUnitate']==$idUnitate and $clasa==$row['cys']){ ?>
      ...
      <center><input class="buton-triforce" type="submit" value="AFISEAZA LISTA CU NOTE"></center>
    </form>
  </div>
