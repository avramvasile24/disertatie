...
if(isset($_REQUEST['update_id']))
{
	try
	{
		...
		$select_stmt = $db->prepare('SELECT * FROM catalog WHERE id =:id'); //sql select query
		$select_stmt->bindParam(':id',$id);
		$select_stmt->execute(); 
		$row = $select_stmt->fetch(PDO::FETCH_ASSOC);
		extract($row);
...
{
	try
	{		$materie	=$_REQUEST['materie'];		//textbox name "txt_name"
		$nota	=$_REQUEST['note'];
		$data=$_REQUEST['data'];
		...
		if(!isset($errorMsg))
		{
			$update_stmt=$db->prepare('UPDATE catalog SET nota_materie=:nota, materie=:materie, data=:data WHERE id=:id'); //sql update query	//bind all parameter
			$update_stmt->bindParam(':id',$id);
			$update_stmt->bindParam(':nota',$nota);
			$update_stmt->bindParam(':materie',$materie);
			$update_stmt->bindParam(':data',$data);
...
<meta name="viewport" content="initial-scale=1.0, maximum-scale=2.0">title>
	...
	<div class="container">
		...
		?>   
			...
			<form method="post" class="form-horizontal" enctype="multipart/form-data">
				...
				<div class="col-sm-6">
				<input type="text" name="materie" class="form-control" value="<?php echo $materie; ?>" required/>
				</div>
				</div>
				...
				<div class="col-sm-6">
				<input type="text" name="note" class="form-control" value="<?php echo $nota_materie; ?>" required/>
				</div>
				</div>
				...
				<a href="dashboard.php" class="btn btn-primary">MENIUL PRINCIPAL</a>
				<a href="func3.php" class="btn btn-primary">CATALOG</a>
				</div>
				</div>
