  ...
  <<script src="js/fullcalendar.min.js">
    ...
    <div class="card-clase">
      <div class="header-pagina" style="margin-bottom: 30px;">
        <h3>&nbsp; FEEDBACK - <b><?php echo $unitate; ?></b></h3>
      </div>

    <div class="card-clase">
      <div class="header-pagina">
      <h3>&nbsp; CAUTA TEME - <b><?php echo $unitate; ?></b></a></h3>
    </div>
    ...
    <div class=" col-lg-12">
        <div class="col-lg-12">
          <div class="panel-body">
            <div class="table-responsive">
              <form action="cautareteme.php" method="POST">
                <div class="form-group"><div class="col-sm-6">
                  <label class="col-sm-3 control-label"><b>CLASA:</b></label>
                  
                    <select name="clasa" id="clasa"> <?php include 'connection1.php';
            $select_stmt = $db->prepare("SELECT * FROM cys WHERE 1");  //sql select query
            $select_stmt->execute();

            while ($row = $select_stmt->fetch(PDO::FETCH_ASSOC)) {
              if ($row != 1) { ?>

                  <option><?php echo $row["cys"]; ?>
                </option> <?php
                    }
                  }
                      ?>
		...
		<label class="col-sm-3 control-label"><b>MATERIE:</b></label>
                    ...
                    <select name="materie" id="materie"> <?php include 'connection1.php';
            $select_stmt = $db->prepare("SELECT * FROM materii WHERE 1");  //sql select query
            $select_stmt->execute();

            while ($row = $select_stmt->fetch(PDO::FETCH_ASSOC)) {
              if ($row != 1) { ?>

                  <option><?php echo $row["materie"]; ?>
                </option> <?php
                    }
                  }
                      ?>
          ...
          <center><input class="buton-triforce" type="submit" value="CAUTARE TEME"></center>
        </div>
		</form>
		</div>
		</div>
