  ...
  <div id="frm" class="panel-parteneri">
    <div class="panel-body">
      <div class="table-responsive">
        <div class="titlu-parteneri" style='color: #FF00FF; font-weight: bold;'>LISTA PARTENERI</div>
        <table class="table table-striped table-bordered table-hover">
          <thead>
            <tr>
              <th style='color: #FF00FF; font-weight: bold;'>PARTENER:</th>
              <th style='color: #FF00FF; font-weight: bold;'>STEMA:</th>
              <th style='color: #FF00FF; font-weight: bold;'>COD ACTIVARE:</th>
              <th style='color: #FF00FF; font-weight: bold;'>NUMAR DE TELEFON: </th>
              <th style='color: #FF00FF; font-weight: bold;'>EMAIL: </th>
              <th style='color: #FF00FF; font-weight: bold;'>ADRESA PARTENER:</th>
              <th style='color: #FF00FF; font-weight: bold;'>RESPONSABIL:</th>
              <th style='color: #FF00FF; font-weight: bold;'>STARE:</th>
              <th style='color: #FF00FF; font-weight: bold;'>EDITEAZA</th>
              <th style='color: #FF00FF; font-weight: bold;'>STERGE</th>
            </tr>
          </thead>
          <tbody>
            <?php
            $select_stmt = $db->prepare("SELECT * FROM parteneri WHERE 1");  //sql select query
            $select_stmt->execute();
            while ($row = $select_stmt->fetch(PDO::FETCH_ASSOC)) {
              if ($row != 1) {
            ?>
                <tr>
                  <td><?php echo $row['nume_partener']; ?></td>
                  <td><embed src="upload/<?php echo $row['stema']; ?>" height="100px" /></td>
                  <td><?php echo '<b>', $row['cod_activare'], '</b> - pt secretariat <br>';
                      echo '<b>', $row['cod_activare_profesor'], '</b> - pt profesori<br>'; 
					   echo '<b>', $row['cod_activare_elevi'], '</b> - pt elevi<br>';
                      echo '<b>', $row['cod_activare_parinti'], '</b> - pt parinti'; ?></td>
                  <td>+40<?php echo $row['contact_telefon'] ?></td>
                  <td><?php echo $row['contact_email'] ?></td>
                  <td><?php echo $row['adresa_partener'] ?></td>
                  <td><?php echo $row['responsabil_unitate']; ?></td>
                  <td><b><?php if ($row['stare_unitate'] == "ACTIV") {
                        echo '<h7 style="color:Green;">', $row['stare_unitate'];
                      } else {
                        echo '<h3 style="color:Red;">', $row['stare_unitate'];
                      } ?></td></b>
                  <td><a href="edit1.php?update_id=<?php echo $row['id']; ?>" class="btn btn-warning">EDITEAZA</a></td>
                  <td><a href="?delete_id=<?php echo $row['id']; ?>" class="btn btn-danger">STERGE</a></td>
                </tr>
            <?php
              }
            }
        ...
        <center>
          <div class="buton-triforce" >
            <a href="func1.php"><i class="fa fa-plus"></i> ADAUGA PARTENERI</a>
          </div>
        </center>
