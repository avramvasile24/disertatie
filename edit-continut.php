     ...
     <li id="mail_notificatoin_bar" class="dropdown">
            <a data-toggle="dropdown" class="dropdown-toggle" href="#">
              ...
              <span class="badge bg-important"><?php  
				...
				while ($row = mysqli_fetch_assoc($result) and $ok==0)
					if ($date==$row['username_inceput']){ echo '1'; $ok=$ok+1;}
				if($ok==0) {echo '0';}
				?></span>
            </a>
            <ul class="dropdown-menu extended inbox">
              ...
              <li>
				...
				$ok=0;
				while ($row = mysqli_fetch_assoc($result) and $ok==0)
					if ($date==$row['username_inceput']){ echo 'MESAJE NOI'; $ok=$ok+1;}
...
<center><div class='buton-triforce'><a href='func4.php'><i class='fa fa-back'></i> REVINO LA CURS</a></div>
	<div class='buton-triforce'><a href='dashboard.php'><i class='fa fa-back'></i> REVINO LA PAGINA PRINCIPALA</a></div>
     

      <div style="padding-top: 100px;"></div>

  <div class="card-elevi">
    <div class="header-pagina" style="margin-bottom: 40px;"> EDITEAZA ACTIVITATI</div>
	<form action="finish-continut.php" method="POST">
    <input class="buton-adauga-note" type="text" name="materie" id="materie" value="<?php  echo $materie;?>">
	    <input class="buton-adauga-note" type="text" name="clasa" id="clasa" value="<?php  echo $clasa;?>">
		   <input class="buton-adauga-note" type="text" name="tip" id="tip" value="<?php  echo $tip;?>">
		    <input class="buton-adauga-note" type="text" name="saptamana" id="saptamana" value="<?php  echo $saptamana;?>">
			<input class="buton-adauga-note" type="text" name="titlu" id="titlu" value="<?php  echo $titlu;?>">
		
	  <?php if($tip=="Lectie"){
		  $sql = "SELECT * FROM continut WHERE 1";
    $result = mysqli_query($conectare, $sql);
    $ok = 0;
    while ($row = mysqli_fetch_assoc($result)) { if($row['titlu']==$titlu){
		echo '<label class="col-sm-12 control-label"><b>Titlu lectiei:</b></label>';
		  echo'<input  type="text" name="titlu1" id="titlu1" value="', $row['titlu'],'">';
		  echo' <label class="col-sm-12 control-label"><b>Continutul lectiei:</b></label>';
		   echo'<input  type="text" name="titlu2" id="titlu2" value="', $row['continut'],'">';
	}
		  
	  }}
	  else { $sql = "SELECT * FROM teme WHERE 1 ";
    $result = mysqli_query($conectare, $sql);
    $ok = 0;
    while ($row = mysqli_fetch_assoc($result)) {if($row['titlu']==$titlu){
				echo '<label class="col-sm-12 control-label"><b>Titlu temei:</b></label>';
		  echo'<input  type="text" name="titlu1" id="titlu1" value="', $row['titlu'],'">';
		  echo' <label class="col-sm-12 control-label"><b>Continutul temei:</b></label>';
	echo'<input  type="text" name="titlu2" id="titlu2" value="', $row['continut'],'">';
	echo' <label class="col-sm-12 control-label"><b>DATA temei:</b></label>';
	echo'<input  type="text" name="date" id="date" value="', $row['data_final'],'">';}}}?>
       

      <center><input class="buton-triforce" type="submit" value="Finalizati modificarea"></center>
    </form>
  </div>
    </div>
    <div class="row">
