...
<body>
  ...
  <section id="container" class="">
    ...
    if (!$conectare) {
	  ...
	  if ($row1['clasa'] == $clasa and $row1['curs'] == $materie) { $id=$row1['id'];}}
...
<center><div class='buton-triforce'><a href='func4.php'><i class='fa fa-back'></i> REVENITI LA CURS</a></div>
  ...
  <div class="card-elevi">
	...
	<form action="edit-continut.php" method="POST">
    <input class="buton-adauga-note" type="text" name="materie" id="materie" value="<?php  echo $materie;?>">
	  ...
	  <?php if($tip=="Lectie"){ echo ' 
      <label class="col-sm-12 control-label"><b>Titlu lectiei:</b></label><select name="titlu" id="titlu">';
    $sql = "SELECT * FROM continut WHERE idCurs= '$id' AND saptamana=$saptamana ";
    $result = mysqli_query($conectare, $sql);
    $ok = 0;
    while ($row = mysqli_fetch_assoc($result)) {
      ...
      ';}else{ echo '
      <label class="col-sm-12 control-label"><b>Titlu lectiei:</b></label><select name="titlu" id="titlu">';
    $sql = "SELECT * FROM teme WHERE idCurs= '$id' AND saptamana=$saptamana ";
    $result = mysqli_query($conectare, $sql);
    $ok = 0;
    while ($row = mysqli_fetch_assoc($result)) {
		echo '<option>',$row['titlu'],'</option>';
      ...
      <center><input class="buton-triforce" type="submit" value="Finalizati editarea"></center>
    </form>
  </div>
    </div>
    <div class="row">
